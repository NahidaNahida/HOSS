...
from pauli import LinearPauliRotations
...
def test_input_generation(n_list, maxTests=None):
    ...
    initial_gates = [0, 1]
    slop_list = np.arange(-5, 5 + 1e-9, 0.5)
    offset_list = np.arange(-math.pi, math.pi + 1e-9, math.pi/8)
    basis_list = ['Y']
            ...
            for basis in basis_list:
                for n in n_list:  
                    ...
                    for initial_state in initial_states:
                        tempNum += 1
                        testInput.append([n, initial_state, slop, offset, basis])
...
def program_running(testOrderListRaw, testInput, version, maxTests=None, faultRatio=0.5, eps_0=1e-10):
    ...
    for testOrder in tqdm(testOrderList):                    
        initial_state, slop, offset, basis = testInput[testOrder][1], testInput[testOrder][2], testInput[testOrder][3], testInput[testOrder][4]
        n = len(initial_state)
        number = int(''.join(map(str, initial_state)), 2)   # the decimal number
        qc_initial = QuantumCircuit(n + 1, n + 1)
        ...
        func = version_selection("LinearPauliRotations", version)
        qc_test = func(n, slop, offset, basis)
        qc_raw = LinearPauliRotations(n, slop, offset, basis)
        ...
        test_case = [n, qc_exp.num_qubits, number, slop, offset, basis, truth]
    ...
    file_name = "LPR_{}_testSuites_(qubit={},fr={},#t={}).csv".format(str(version), 
    ...
    with open(file_name, mode='w', newline='') as file:
        ...
        header = ['n', '# qubits','number', 'slop', 'offset', 'basis', 'if_pass']
...
if __name__ == '__main__':
    n_list = [9]
    for version in ['v2']:
        testOrderListRaw, testInput = test_input_generation(n_list, 2000)
