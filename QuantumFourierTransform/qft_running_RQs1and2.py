from qft_test_process_RQs1and2 import *
import os
import pandas as pd
import csv
    ...
    for num_qubits in num_qubits:
        for program_version in version_list:
            ...
            for test_oracle in test_oracles:
                ...
                if toler_err == 0.05:
                    file_name = "QFT_{}_RQ1_{}.csv".format(program_version, test_oracle)
                else:
                    file_name = "QFT_{}_RQ1_{}_err={}.csv".format(program_version, test_oracle, toler_err)
    ...
    for num_qubits in num_qubits:
        for program_version in version_list:
            ...
            for test_oracle in test_oracles:
                ...
                file_name = "QFT_" + str(program_version) + "_RQ2_" + str(test_oracle) + ".csv"
                with open(file_name, mode='w', newline='') as file:
                    writer = csv.writer(file)
                    header = ['shot', 'time_list', 'fault_list', 'ave_time', 'ave_fault', 'std_time', 'std_fault']
                    writer.writerow(header)
