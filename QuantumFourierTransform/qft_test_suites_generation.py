...
def test_input_generation(n_list, maxTests=None):
    ...
    for n in n_list:
        ...
        for initial_state in initial_states:
            for if_swap in [True, False]:
                tempNum += 1
                testInput.append([n, initial_state, if_swap])
...
def program_running(testOrderListRaw, testInput, version, maxTests=None, faultRatio=0.5, eps_0=1e-10):
    ...
    for testOrder in tqdm(testOrderList):                    
        ...
        func = version_selection("QFT", version)
        qc_test = func(n, do_swaps=if_swap)
        qc_raw = QFT(n, do_swaps=if_swap)
        ...
        test_case = [n, qc_exp.num_qubits, number, int(if_swap), truth]
    ...
    file_name = "QFT_{}_testSuites_(qubit={},fr={},#t={}).csv".format(str(version), 
    ...
    with open(file_name, mode='w', newline='') as file:
        ...
        header = ['n', '# qubits','number', 'if_swap', 'if_pass']
